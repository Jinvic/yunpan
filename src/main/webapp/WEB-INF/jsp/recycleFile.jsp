...
<head>
	...
	function mouseOver(i){
		...
		$(".addSrc").eq(2*i).attr('src',"${pageContext.request.contextPath }/img/refresh1.png");
		$(".addSrc").eq(2*i+1).attr('src',"${pageContext.request.contextPath }/img/delete.png");
	}
	function mouseOut(){
	...
	function revertFile(obj){
		...
		$.post("${pageContext.request.contextPath }/file/revertDirectory.action", {"fileId":id}, function(data){
			layer.msg(data.msg);
	...
	function revertAllFiles(){
		...
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath }/file/revertDirectory.action",
			data:{
				"fileId":fileId,
			},
	...
	function delAllFiles(){
		layer.confirm('确认清空回收站？', {
			  btn: ['确认','返回'] //按钮
				...
				$.ajax({
					type:"POST",
					url:"${pageContext.request.contextPath }/file/delAllRecycle.action",
					data:{
					},
					success:function(data){
						layer.msg(data.msg);
	...
	function delFile(obj){
		layer.confirm('确认删除？', {
			  btn: ['确认','返回'] //按钮
				...
				var id = $(obj).parent().parent().children(".oneCheck").attr("id");
				$.post("${pageContext.request.contextPath }/file/delRecycle.action", {"fileId":id}, function(data){
					layer.msg(data.msg);
					window.location.reload();
				});
			}, function(){
		...
		<c:choose>
			<c:when test="${findDelFile == null}">
				<div id="empty" style="display:block;">
					<img src="${pageContext.request.contextPath }/img/empty.png" />
			...
			<c:when test="${findDelFile != null}">
				<div style="display:block" id="collection">
					<ul>
						<li id="list-info" class="list-border"><span class="para" id="warn">提示:回收站不占用云盘空间，文件保存10天后
						...
						<li class="list-border">
							...
							<div style="display: none;border: 0;" class="list-table" id="div2">
								...
								<span  onclick="revertAllFiles()" class="para1" id="revert" style="margin-top:4px;float: left;margin-left: 1.5%;">
									<img src="${pageContext.request.contextPath }/img/refresh1.png"/>
						...
						<c:forEach items="${findDelFile}" var="delFile" varStatus="index">
							<li class="list-tr" class="list-border" style="cursor:pointer;" onmouseover="mouseOver(${index.index})" onmouseout="mouseOut()">
								...
								<span style="width: 49%;height:100%;float:left;">
									<img style="float: left;margin: 3px 4px 0 5px;" src="${pageContext.request.contextPath }/img/picture.png" />
